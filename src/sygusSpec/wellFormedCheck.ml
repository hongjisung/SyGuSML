...
let inv_constraint_well_formed_check
  : Ast.cmd list -> int -> (bool * int * Ast.sorted_var list) =
  let bool_sort : Ast.sort = Sort (SymbolIdentifier (Symbol "Bool")) in
  fun cmdlist n ->
    ...
    let svl, srt  = find_svlist_and_sort_from_SynthFun   cmdlist s |> opt_resolve_svlist_and_sort in
    let n_s       = find_and_count_argnum_from_SynthFun  cmdlist s in
    ...
    let is_well_formed = (srt = bool_sort) && (n_s = n_spre) && (n_s = n_spost) && (n_s * 2 = n_strans) in
    (is_well_formed, n_s, svl)
